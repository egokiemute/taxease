...
const GalleryCompnent = ({ entryId }: Props) => {
  ...
  const GalleryObjects: GalleryData[] = EntryData?.content
    ? JSON.parse(EntryData.content)
    : [];
    ...
    if (files) {
      ...
      for (let i = 0; i < fileArray.length; i++) {
        ...
        try {
          ...
          updateEntry(
            entryId,
            JSON.stringify(updatedNewFileObject),
            EntryData?.extra1,
            EntryData?.extra2
          );
        } catch (error) {
  ...
  return (
    <div className="flex flex-col mt-20">
      <div
        ...
        id="myDiv"
        className={`relative hover:border-2 transition-all duration-100 ease-in ${
          EntryData?.extra2 === "large" ? "max-w-screen-lg" : "max-w-screen-md"
        } ${
          clickedElement ? "border-2 border-pink-500" : ""
      ...
      >
        <div
          ...
          className={`flex items-center justify-center min-h-[400px] ${
            !EntryData?.content ? "bg-gray-50" : "bg-white"
          } aspect-video transition-all ease-in duration-200 rounded-xl m-4`}
        >
          {GalleryObjects.length > 0 ? (
            ...
            <label
              onClick={handleClick}
              htmlFor=""
              className="flex flex-col items-center justify-center min-h-[400px] aspect-video"
            >
        ...
        <div
          className={`absolute top-0 -right-14 w-10 p-2 py-4 rounded-full flex flex-col items-center justify-center gap-3 bg-white shadow-xl ${
            isMenuOpen && selectedEntryId === entryId ? "" : "hidden"
          }`}
        >
        ...
        <div className="h-[90px] w-full flex items-center justify-center overflow-x-auto">
          <div className="flex items-center space-x-3">
            {GalleryObjects.length > 0 &&
              GalleryObjects.map((fileObj, index) => (
                <div
                  ...
                  className={`w-[80px] h-[70px] rounded-lg border-neutral-300 flex items-center justify-center cursor-pointer space-x-2 relative  ${
                ...
                >
                  ...
                  <div
                    ...
                    className="p-1 rounded-full bg-white absolute top-0 right-0"
                  >
            ...
            <div
              onClick={handleClick}
              className="w-[80px] h-[70px] border-dashed border-[1.5px]  rounded-lg border-neutral-300 flex items-center justify-center hover:border-pink-500 cursor-pointer"
            >
      ...
      <div className="flex opacity-0 hover:opacity-100 transition-all duration-150 ease-in items-center mt-10">
        ...
        <button
          ...
          className="p-3 rounded-full bg-pink-500 flex items-center text-white"
        >
          <svg
            data-v-351edcb1=""
            viewBox="0 0 24 24"
